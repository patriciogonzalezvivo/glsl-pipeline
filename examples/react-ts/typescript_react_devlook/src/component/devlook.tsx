...
import { BufferGeometry, Light, LightShadow, PerspectiveCamera, PlaneGeometry, Vector3 } from 'three'
...
import { GlslPipelineClass, Lights } from "glsl-pipeline/src/types-helper"
...
export default function Devlook() {
    ...
    const devlookRef = useRef<GlslPipelineClass | null>(null);
    const floorRef = useRef<BufferGeometry | null>(null);
    const lightRef = useRef<Light<LightShadow<PerspectiveCamera>> | null>(null);
    const { camera, gl, scene } = useThree();

    const sceneLoader = useLoader(OBJLoader, '/dragon.obj', undefined, (xhr) => {
    ...
    useEffect(() => {
        ...
        if (devlookRef.current && sceneLoader.children.length > 0) {
            ...
            devlookRef.current.setLight(lightRef.current as Lights);
        }
...
void main(void) {
    ...
    return (
        <>
            <directionalLight color={new Color(0xffffff)} ref={lightRef as unknown as Ref<DirectionalLight>} position={new Vector3(0, 10, 8)} castShadow lookAt={new Vector3(0, 0, 0) as never} intensity={1.0} />
            <mesh receiveShadow>
                <planeGeometry ref={floorRef as unknown as Ref<PlaneGeometry>} args={[5, 5, 1, 36]}/>
